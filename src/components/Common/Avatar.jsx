  ...
  return( 
    <div className="avatar-container flex relative w-20 h-7">
      ...
      {messageCount > MAX_DISPLAY &&
        <div className={"avatar avatar-more w-7 h-7 rounded-full border text-black border-gray-200 flex items-center justify-center text-xs bg-white absolute left-12"}>{`+${messageCount - MAX_DISPLAY}`}</div>
      }
    </div>
  );
};
